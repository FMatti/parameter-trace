...
\begin{proof}
    ...
    \end{equation}
    \todo{In particular, its moments are also bounded with this.} The $p$-th moment of the second term can be processed with standard matrix norm manipulations to
    \begin{align}
        ...
        &\leq \sqrt{k} \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \rVert _2^2\right] \mathbb{E}^{p}\left[ \lVert \mtx{\Omega}_1^{\dagger} \rVert _2^2  \right]  && \text{(independence $\mtx{\Omega}_1, \mtx{\Omega}_2$)} \notag \\
        &= \sqrt{k} \mathbb{E}^{2p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \rVert _2 \right]^2 \mathbb{E}^{2p}\left[  \lVert \mtx{\Omega}_1^{\dagger} \rVert _2 \right]^2 && \text{(definition of $\mathbb{E}^{k}[\cdot]$)}
        \label{equ:nystrom-proof-processed-tail}
    \end{align}
    We can apply \reflem{lem:spectral-norm-moment} to $\mtx{\Lambda}_2^{\sfrac{1}{2}}$ to bound the first term with
    \begin{equation}
        \mathbb{E}^{2p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \rVert _2 \right]
        \leq \sqrt{2p + n_{\mtx{\Omega}}} \left( 3 \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _2 + \frac{1}{\sqrt{n_{\mtx{\Omega}}}} \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _F \right).
    ...
    \begin{align}
        \mathbb{E}^{2p}\left[ \lVert \mtx{\Omega}_1^{\dagger} \rVert _2 \right]
        &= \mathbb{E}\left[ \lVert ( \mtx{\Omega}_1 \mtx{\Omega}_1^{\top} )^{-1} \rVert _2^{p} \right]^{\sfrac{1}{2p}}  \notag \\
        &\leq \left( 1 + \frac{2p}{n_{\mtx{\Omega}} - k + 1 - 2p} \right)^{\sfrac{1}{2p}} \left( \frac{1}{(n_{\mtx{\Omega}} - m + 1)!}\right)^{\frac{1}{n_{\mtx{\Omega}} - m + 1}} \left( \frac{k + n_{\mtx{\Omega}}}{2}\right)^{\sfrac{1}{2}} \notag \\
        &\leq \frac{e^2}{\sqrt{2}} \frac{\sqrt{k + n_{\mtx{\Omega}}}}{n_{\mtx{\Omega}} - k + 1}
        \label{equ:pinv-spectral-norm-bound}
    \end{align}
    with the last steps following from the expansion of the exponential function $e^n = 1 + n + \dots + n^n/n! + \dots$ for $n \in \mathbb{N}$, i.e. $e^n \geq 1 + n$ and $e^n \geq n^n / n!$ for all $n \in \mathbb{N}$, from which $(n + 1)^{\sfrac{1}{n}} \leq e$ and $\left( 1/n! \right)^{\sfrac{1}{n}} \leq e/n$ follow respectively. Hence, plugging \refequ{equ:spectral-norm-bound-applied} and  \refequ{equ:pinv-spectral-norm-bound} into \refequ{equ:nystrom-proof-processed-tail}, and using $(a + b)^2 \leq 2a^2 + 2b^2$ for $a, b > 0$ gives us
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _{(4)}^2 \right]
        \leq \sqrt{k} \frac{e^4}{2} \frac{(k + n_{\mtx{\Omega}})(2p + n_{\mtx{\Omega}})}{(n_{\mtx{\Omega}} - k + 1)^2}  \left( 9 \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _2^2 + \frac{1}{n_{\mtx{\Omega}}} \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _F^2 \right)
    \end{equation}
    ...
    \begin{align}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _{(4)}^2 \right]
        &\leq \sqrt{k} \frac{e^4}{2} \frac{(k + n_{\mtx{\Omega}})(2p + n_{\mtx{\Omega}})}{(n_{\mtx{\Omega}} - k + 1)^2}  \left( \frac{9}{k} \Trace(\mtx{B}) + \frac{1}{n_{\mtx{\Omega}}} \Trace(\mtx{B}) \right) \notag \\
        &= \left( \frac{e^4}{2} \frac{(k + n_{\mtx{\Omega}})(2p + n_{\mtx{\Omega}})}{(n_{\mtx{\Omega}} - k + 1)^2} \frac{9n_{\mtx{\Omega}} + k}{n_{\mtx{\Omega}}}\right) \frac{\Trace(\mtx{B})}{\sqrt{k}} 
        \label{equ:nystrom-proof-tail-bound}
    \end{align}
    To match the decay rate of the first term \refequ{equ:nystrom-proof-frobenius-trace}, we need to ensure that the term enclosed in parentheses is constant. We are free to choose $p > 1$ and $k \in \mathbb{N}$ such that $n_{\mtx{\Omega}} \geq k + 2p$. This is the case if we let $k = n_{\mtx{\Omega}}/2$ and $p = n_{\mtx{\Omega}}/4$. Then, using the triangle inequality for $\mathbb{E}^{p}[\cdot]$ and inserting \refequ{equ:nystrom-proof-tail-bound} along with \refequ{equ:nystrom-proof-frobenius-trace} in \refequ{equ:nystrom-proof-persson-bonud} we obtain
    \begin{equation}
    ...
    \end{equation}
    where $c = 1 + 9 \sqrt{2} e^4 (9 + \frac{1}{2})$.
...
\begin{theorem}{Nyström++ trace estimator for parameter-dependent matrices}{nystrom-pp}
    Let $\mtx{B}(t) \in \mathbb{R}^{n \times n}$ be symmetric positive semi-definite and continuously depend on $t \in [a, b]$. If $n_{\mtx{\Psi}} = n_{\mtx{\Omega}} = \mathcal{O}(\varepsilon^{-1} \log(2\delta^{-1})^2 + \log(2\delta^{-1}))$ with \emph{even} $n_{\mtx{\Omega}}$, then for $\delta \in (0, e^{-\sfrac{1}{2}}/2)$ and $\varepsilon > 0$ with probability at least $1 - \delta$
    \begin{equation}
