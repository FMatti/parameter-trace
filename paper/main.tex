...
\begin{itemize}
    ...
    \item Trace estimation in the parameter-dependent matrices (Nyström++ but no analysis \cite{lin-2017-randomized-estimation}, only low-rank approximation \cite{kressner-2023-randomized-lowrank}, dynamic trace estimation for sequence $\mtx{A}_1, \dots, \mtx{A}_m$ of similar matrices \cite{woodruff-2022-optimal-query, dharangutte-2021-dynamic-trace}) their algorithm provides trace approximations $a_1, \dots , a_m$ for $\Trace(\mtx{A}_1), \dots, \Trace(\mtx{A}_m)$ such that, for each $i \in 1, \ldots, m$,
    \begin{equation}
        \mathbb{P}(|a_i - \Trace(\mtx{A}_i)| \geq \epsilon)\leq \delta.
    \end{equation}
    drawback of this type of guarantee is that when we aim for $\epsilon$ accuracy simultaneously for all $|a_i-\Trace(\mtx{A}_i)|$, the probability will grow linearly with $m$ by the union bound. On the other hand, in our setting, instead of discretizing $[a, b]$ with $t_1, \ldots, t_m$ and looking at the error $|a(t_i)-\Trace(\mtx{A}(t_i))|$ for each $i$, a much more natural way to quantify the error would be to look at $\int_{a}^{b} | a(t) - \mtx{A}(t) |~\mathrm{d}t$, where $a(t)$ is the trace estimation in each $t \in [a, b]$.
...
\begin{itemize}
    \item Same random matrix for all $t$, which allows for favorable scaling in the number of evaluation points
    ...
    \item A simple and efficient algorithm for estimating spectral densities using any of the three estimators with Chebyshev expansion, including theoretical guarantees
\end{itemize}
}
...
\end{equation}
which was originally proposed by \cite{lin-2017-randomized-estimation}. %We can interpret this estimator as an interpolation between the trace of the Nyström approximation and the Girard-Hutchinson estimator.
...
\begin{remark}
    If additionally $\mtx{A}(t)$ is nonzero and symmetric positive semi-definite for all $t \in [a, b]$, then it follows from the proof of \refthm{thm:hutchinson} that with probability at least $1 - \delta$
    \begin{equation}
