...
\begin{itemize}
    ...
    \item Check that everywhere correct/consistent orders $\mathcal{O}(\varepsilon^{-2} + \log(\delta^{-1})$)
    \item 
\end{itemize}
}
...
\begin{proof} 
    ...
    Now we make the transition to the continuous. Following \cite{kressner-2023-randomized-lowrank}, we can show that the error $\lVert \mtx{B}(t) - \Nystr{\mtx{\Omega}}{\mtx{B}}(t) \rVert _F$ is measurable. Therefore, Minkowski's integral inequality \cite[theorem 202]{hardy-1952-inequalities} allows us to apply \refequ{equ:hutchinson-trace-onequery-fixed} in the continuous setting: 
    \begin{equation}
    ...
    \end{equation}
    from which follows the second part of the theorem when $n_{\mtx{\Psi}} = (2^5 e^{\sfrac{1}{2}} \varepsilon \log(\delta^{-1}))^2$.
...
\begin{remark}
    ...
    \end{equation}
    for any $k,p \in \mathbb{N}, p \geq 2$.
\end{remark}
...
\begin{lemma}{Spectral norm moments of non-standard Gaussian random matrix}{spectral-norm-moment}
    The matrix $\mtx{A} \in \mathbb{R}^{m \times m}$ and the standard Gaussian random matrix $\mtx{\Omega} \in \mathbb{R}^{m \times k}$ satisfy for all $p \in \mathbb{N}, p \geq 2$ 
    \begin{equation}
...
\begin{remark}
    ...
    \end{equation}
    for any $k,p \in \mathbb{N}, p \geq 2$. Indeed, for $p=2$ this bound is slightly less tight than its equivalent in \cite[lemma B.1]{tropp-2023-randomized-algorithms}.
\end{remark}

\begin{proof}
    From the proof of \cite[lemma B.1]{tropp-2023-randomized-algorithms} we know that for $p \geq 2$
    \begin{equation}
        \mathbb{E}^{\sfrac{p}{2}}\left[ \lVert \mtx{A} \mtx{\Omega} \rVert _2^2 \right]
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq \mathbb{E}^{p}\left[ \sqrt{k} \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _2^2 \right]  && \text{($\lVert \mtx{A} \rVert _F \leq \sqrt{k} \lVert \mtx{A} \rVert _2$)} \notag \\
    ...
    \end{align}
    We can apply \reflem{lem:spectral-norm-moment} with $\mtx{A} = \mtx{\Lambda}_2^{\sfrac{1}{2}}$ to bound
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \rVert _2^2 \right]
        \leq (n_{\mtx{\Omega}} + 2p) \left( 3 \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _2^2 + \frac{1}{n_{\mtx{\Omega}}} \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _F^2 \right).
        \label{equ:spectral-norm-bound-applied}
    \end{equation}
    The moments of $\lVert \mtx{\Omega}_1^{\dagger} \rVert _2$ can be bound with help of the proof of \cite[lemma B.3]{tropp-2023-randomized-algorithms} when $n_{\mtx{\Omega}} \geq k + 2p$
    \begin{align}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Omega}_1^{\dagger} \rVert _2^2 \right]
        &= \mathbb{E}\left[ \lVert ( \mtx{\Omega}_1 \mtx{\Omega}_1^{\top} )^{-1} \rVert _2^{p} \right]^{\sfrac{1}{p}}  \notag \\
        &\leq \left( 1 + \frac{2p}{n_{\mtx{\Omega}} - k + 1 - 2p} \right)^{\sfrac{1}{p}} \left( \frac{1}{(n_{\mtx{\Omega}} - k + 1)!}\right)^{\frac{2}{n_{\mtx{\Omega}} - k + 1}} \left( \frac{n_{\mtx{\Omega}} + k}{2}\right) \notag \\
        &\leq \frac{e^4}{2} \frac{n_{\mtx{\Omega}} + k}{(n_{\mtx{\Omega}} - k + 1)^2}.
        \label{equ:pinv-spectral-norm-bound}
    \end{align}
    The last step follows from the expansion of the exponential function $e^n = 1 + n + \dots + n^n/n! + \dots$ for $n \in \mathbb{N}$ to see $e^n \geq 1 + n$ and $e^n \geq n^n / n!$, from which $(1 + n)^{\sfrac{1}{n}} \leq e$ and $\left( 1/n! \right)^{\sfrac{1}{n}} \leq e/n$ follow respectively. Hence, inserting \refequ{equ:spectral-norm-bound-applied} and  \refequ{equ:pinv-spectral-norm-bound} in \refequ{equ:nystrom-proof-processed-tail} gives
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _{(4)}^2 \right]
    ...
    \end{equation}
    We can identify $\lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _2^2 = \lambda_{k+1}$ and $\lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _F^2 = \Trace(\mtx{\Lambda}_2)$, and just as in the proof of \cite[lemma 3.1]{meyer-2021-hutch-optimal} we bound $\lambda_{k+1} \leq \Trace(\mtx{B})/k$ and $\Trace(\mtx{\Lambda}_2) \leq \Trace(\mtx{B})$ to get 
    \begin{align}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _{(4)}^2 \right]
        &\leq \sqrt{k} \frac{e^4}{2} \frac{(k + n_{\mtx{\Omega}})(2p + n_{\mtx{\Omega}})}{(n_{\mtx{\Omega}} - k + 1)^2}  \left( \frac{3}{k} \Trace(\mtx{B}) + \frac{1}{n_{\mtx{\Omega}}} \Trace(\mtx{B}) \right) \notag \\
        &= \frac{e^4}{2} \left( \frac{(k + n_{\mtx{\Omega}})(2p + n_{\mtx{\Omega}})}{(n_{\mtx{\Omega}} - k + 1)^2} \frac{3n_{\mtx{\Omega}} + k}{n_{\mtx{\Omega}}\sqrt{k}} \right) \Trace(\mtx{B}).
        \label{equ:nystrom-proof-tail-bound}
    \end{align}
    To match the decay rate of the first term \refequ{equ:nystrom-proof-frobenius-trace}, we need to ensure the term enclosed in parentheses is of order $\mathcal{O}(1 / \sqrt{k})$. We are free to choose $p > 1$ and $k \in \mathbb{N}$ such that $n_{\mtx{\Omega}} \geq k + 2p$. This is the case if we let $k = n_{\mtx{\Omega}}/2$ and $p = n_{\mtx{\Omega}}/4$, which will make the term in parentheses become $9\sqrt{2}(3 + 1/2) / \sqrt{n_{\mtx{\Omega}}}$. Then, using the triangle inequality for $\mathbb{E}^{p}[\cdot]$ and inserting \refequ{equ:nystrom-proof-tail-bound} along with \refequ{equ:nystrom-proof-frobenius-trace} in \refequ{equ:nystrom-proof-persson-bonud} we obtain
    \begin{equation}
    ...
    \begin{align}
        ...
        &\stackrel{\text{\cite{hardy-1952-inequalities}}}{\leq} \int_{a}^{b} \mathbb{E}^{\sfrac{n_{\mtx{\Omega}}}{4}} \left[\lVert \mtx{B}(t) - \Nystr{\mtx{\Omega}}{\mtx{B}}(t) \rVert _F \right] ~\mathrm{d}t \notag \\
...
\begin{theorem}{Nyström++ trace estimator for parameter-dependent matrices}{nystrom-pp}
    Let $\mtx{B}(t) \in \mathbb{R}^{n \times n}$ be symmetric positive semi-definite and continuously depend on $t \in [a, b]$. If $n_{\mtx{\Psi}} = n_{\mtx{\Omega}} = \mathcal{O}(\varepsilon^{-1} \log(2\delta^{-1})^2)$ with \emph{even} $n_{\mtx{\Omega}}$, then for $\delta \in (0, e^{-\sfrac{1}{2}}/2)$ and $\varepsilon \in (0, 1)$ with probability at least $1 - \delta$
    \begin{equation}
...
Comparing \refthm{thm:nystrom-pp} with its equivalent for constant matrices \cite[theorem 3.4]{persson-2022-improved-variants}, we notice a less favorable scaling of the required number of random vectors $n_{\mtx{\Omega}}$ and $n_{\mtx{\Psi}}$ in terms of $\log(2\delta^{-1})$.
...
The proof specializes Bernstein's theorem for Chebyshev interpolation to the case of a Gaussian smoothing kernel \refequ{equ:smoothing-kernel}. It loosely follows the proof give in the preprint of \cite[theorem 2]{lin-2017-randomized-estimation}, but attains a slightly different dependence on $\sigma$.
...
\end{equation}
for which we can first precompute the $n_{\mtx{\Omega}} \times n_{\mtx{\Omega}}$ matrices $\mtx{\Omega}^{\top} T_l(\mtx{A}) \mtx{\Omega}$ for $l=0, \dots, m$ using the recurrence \refequ{equ:chebyshev-recurrence} and then inexpensively sum them up with the corresponding coefficients $\mu_l(t)$ for all $t$ in which we want to evaluate the approximation. Thanks to the fast and exact squaring of Chebyshev expansions (cf. \refalg{alg:chebyshev-squaring}), the second parenthesized term in \refequ{equ:cyclic-property} can be approximated in an equivalent way. Unlike expanding the squared smoothing kernel $g_{\sigma}^2$ separately \cite{lin-2017-randomized-estimation}, this approach is consistent, which we have observed to make a noticeable difference in terms of accuracy, and which enables provability. Similar observations for the case when $\mtx{\Psi}$ is nonzero lead to the following algorithm.
...
What we have found to be crucial for a successful approximation of the full spectral density was the detection of regions wherein lie no eigenvalues, i.e. regions where the spectral density nearly vanishes. For small smoothing parameters $\sigma$, the rapid decay of the Gaussian $g_{\sigma}$ will cause $\mtx{K}(t_i) \approx g_{\sigma}(t_i\mtx{I}_n - \mtx{A})$ to be close to the zero matrix for parameter values $t_i$ located in such regions. Hence, on line \reflin{lin:4-nystromchebyshev-nystrom-pp} we would compute the pseudoinverse of an almost zero matrix, which can be extremely unstable. Notice that $\Trace(\mtx{K}_1(t_i)) / n_{\mtx{\Omega}}$ is the $n_{\mtx{\Omega}}$-query Girard-Hutchinson estimator. We may use it as a rough indicator whether $t_i$ lies within a region where the spectral density nearly vanishes, and subsequently set $\widetilde{\phi}_{\sigma}(t_i) = 0$ instead of computing the full expression on \reflin{lin:4-nystromchebyshev-nystrom-pp}.
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq (1 + \widetilde{\varepsilon}) 2 \underline{E}_{\sigma, m} + \widetilde{\varepsilon}. && \text{(\refthm{thm:chebyshev-error} and normalization)} \notag \\
    \end{align}
...
\end{equation}
where $\lambda_{(1)}, \dots, \lambda_{(n)}$ denote the eigenvalues of $\mtx{A}$ sorted by increasing distance from spectral-parameter $t$, such that $\sigma_1(t) \geq \dots \geq \sigma_n(t)$. Consequently, by using the closed-form expression of the eigenvalues \refequ{equ:gaussian-kernel-eigenvalues}, we may upper bound the numerical rank of a Gaussian smoothing kernel as
\begin{equation}
...
\begin{equation}
    r_{\varepsilon}(g_{\sigma}(t\mtx{I}_n - \mtx{A})) \lessapprox n d_{\varepsilon, \sigma} \approx n \sigma.
    \label{equ:gaussian-kernel-numerical-rank-uniform}
\end{equation}
Hence, choosing $n_{\mtx{\Omega}} = \mathcal{O}(n \sigma)$ will approximations which are significantly better than \refthm{thm:chebyshev-nystrom}.
...
\todo{Explain experimental setup.}
