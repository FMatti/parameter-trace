...
\begin{lemma}{Moment of spectral norm of non-standard Gaussian random matrix}{ose}
    The matrix $\mtx{\Sigma} = \operatorname{diag}(\sigma_1, \dots, \sigma_m) \in \mathbb{R}^{m \times m}$ with $\sigma_1 \geq \dots \geq \sigma_m \geq 0$ and the standard Gaussian random matrix $\mtx{\Omega} \in \mathbb{R}^{m \times k}$ satisfy for all $p \in \mathbb{N}$ 
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Sigma} \mtx{\Omega} \rVert _2 \right]
        \leq  \sqrt{k + p} \left( 3 \lVert \mtx{\Sigma} \rVert _2 + \frac{1}{\sqrt{k}} \lVert \mtx{\Sigma} \rVert _F \right).
    \end{equation}
    %\begin{equation}
...
\begin{proof}
    First, consider the above moment of a vector $\vct{\omega} \in \mathbb{R}^{m}$. We can rewrite
    \begin{equation}
    ...
    \begin{align}
        ...
        &\leq \mathbb{E}^{\sfrac{p}{2}}\left[ \sum_{i=1}^{\ell} \sigma_{1 + (i-1)k}^2 \sum_{j=1}^{k} \omega_{j + (i-1)k}^2 \right] && \text{($\sigma_{j + (i-1)k}^2 \leq \sigma_{1 + (i-1)k}^2$)} \notag \\
        &\leq \sum_{i=1}^{\ell} \sigma_{1 + (i-1)k}^2 \cdot \mathbb{E}^{\sfrac{p}{2}}\left[ \sum_{j=1}^{k} \omega_{j + (i-1)k}^2 \right] && \text{(triangle inequality)} \notag \\
        &\leq \left( \lVert \mtx{\Sigma} \rVert _2^2 + \frac{1}{k} \lVert \mtx{\Sigma} \rVert _F^2 \right) 2 \left( \frac{\Gamma(\frac{k + p}{2})}{\Gamma(\frac{k}{2})} \right)^{\sfrac{2}{p}} && \text{(\refequ{equ:singular-value-group-bound} and $\chi_k^2$ moments \cite[theorem 3.3.2]{hogg-2013-introduction-mathematical})} \notag \\
    ...
    Now, we consider the matrix case. From the proof of \cite[lemma B.1]{tropp-2023-randomized-algorithms} we know that 
    \begin{equation}
        \mathbb{E}^p\left[ \lVert \mtx{\Sigma} \mtx{\Omega} \rVert _2 \right]
    ...
    \end{equation}
    for independent standard Gaussian random vectors $\vct{\omega}_1 \in \mathbb{R}^{k}$ and $\vct{\omega}_2 \in \mathbb{R}^{m}$. Both moments can be bound using \refequ{equ:matvec-spectral-norm-moment-bound}, one of which with $\mtx{\Sigma} = \mtx{I}_k$, to get
    \begin{equation}
        \mathbb{E}^p\left[ \lVert \mtx{\Sigma} \mtx{\Omega} \rVert _2 \right]
        \leq \sqrt{k + p} \left( 3 \lVert \mtx{\Sigma} \rVert _2 + \frac{1}{\sqrt{k}} \lVert \mtx{\Sigma} \rVert _F \right).
    \end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{bmatrix}
    \end{equation}
    for some \todo{$k \leq n/2$}. Further, for a standard Gaussian random matrix $\mtx{\Omega} \in \mathbb{R}^{n \times 2k}$ we define $\mtx{\Omega}_1 = \mtx{U}_1^{\top} \mtx{\Omega} \in \mathbb{R}^{k \times 2k}$ and $\mtx{\Omega}_2 = \mtx{U}_2^{\top} \mtx{\Omega} \in \mathbb{R}^{(n - k) \times 2k}$, which are likewise standard Gaussian.
    ...
    \end{align}
    We can apply \reflem{lem:ose} with $\mtx{\Sigma} = \mtx{\Lambda}_2^{\sfrac{1}{2}}$ to bound the first term with
    \begin{equation}
