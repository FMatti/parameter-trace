...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq \left( \lVert \mtx{\Sigma} \rVert _2^2 + \frac{1}{k} \lVert \mtx{\Sigma} \rVert _F^2 \right) (k + p). && \text{(\refequ{equ:singular-value-group-bound} and \reflem{lem:gamma})}
    \end{align}
...
This theorem shows that when $0 < \sigma \ll 1$, if we choose $m = \mathcal{O}(\sigma^{-1}(\log(\varepsilon^{-1}) + \log(\sigma^{-1})))$, the error we induce with the Chebyshev expansion is at most $\mathcal{O}(\varepsilon)$.
...
\begin{algo}{Chebyshev-Nyström++}{nystrom-chebyshev-pp}
\begin{algorithmic}[1]
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq (1 + \varepsilon) 2 \underline{E}_{\sigma, m} + \varepsilon. && \text{(\refthm{thm:chebyshev-error} and normalization)} \notag \\
    \end{align}
    From \reflem{lem:non-negative-chebyshev-error} we see that if $\sigma \ll 1$, choosing $m = \mathcal{O}(\sigma^{-1}(\log(\varepsilon^{-1}) + \log(\sigma^{-1})))$ ensures $\underline{E}_{\sigma, m} \leq \varepsilon$, and thus, since $\mathcal{\varepsilon} \leq 1$,
    \begin{equation}
...
Hence, choosing \todo{how discuss how to choose n Omega}
