...
\begin{proof} 
    ...
    \end{equation}
    from which follows the second part of the theorem when inserting $n_{\mtx{\Psi}} = \mathcal{O}(\varepsilon^{-2} \log(\delta^{-1})^2)$.
...
\begin{lemma}{Spectral norm moments of non-standard Gaussian random vector}{spectral-norm-moment-vector}
    ...
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{A} \vct{\omega} \rVert _2^2 \right]
        \leq  (k + 2p) \left( \lVert \mtx{A} \rVert _2^2 + \frac{1}{k} \lVert \mtx{A} \rVert _F^2 \right).
    \end{equation}
    %\begin{equation}
...
\begin{remark}
    \todo{Add remark (need to maybe convert back appendix lemma to $p/2$)}
    \begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Sigma} \vct{\omega} \rVert _2^2 \right]
        = \mathbb{E}^{p}\left[ \sum_{i=1}^{m} \sigma_i^2 \omega_i^2 \right].
    ...
    \begin{align}
        \mathbb{E}^{p}\left[ \sum_{i=1}^{m} \sigma_i^2 \omega_i^2 \right]
        &= \mathbb{E}^{p}\Bigg[ \sum_{i=1}^{\ell} \sum_{j=1}^{k} \underbrace{\sigma_{j + (i-1)k}^2}_{\leq \sigma_{1 + (i-1)k}^2} \omega_{j + (i-1)k}^2 \Bigg] && \text{(separate sum into groups)} \notag \\
        &\leq \mathbb{E}^{p}\left[ \sum_{i=1}^{\ell} \sigma_{1 + (i-1)k}^2 \sum_{j=1}^{k} \omega_{j + (i-1)k}^2 \right] && \text{($\sigma_{j + (i-1)k}^2 \leq \sigma_{1 + (i-1)k}^2$, $\forall i, j$)} \notag \\
        &\leq \sum_{i=1}^{\ell} \sigma_{1 + (i-1)k}^2 ~ \mathbb{E}^{p}\left[ \sum_{j=1}^{k} \omega_{j + (i-1)k}^2 \right] && \text{(triangle inequality)} \notag \\
        ...
        &\leq \left( \lVert \mtx{\Sigma} \rVert _2^2 + \frac{1}{k} \lVert \mtx{\Sigma} \rVert _F^2 \right) (k + 2 p). && \text{(\refequ{equ:singular-value-group-bound} and \reflem{lem:gamma})}
    \end{align}
...
\begin{lemma}{Spectral norm moments of non-standard Gaussian random matrix}{spectral-norm-moment}
    ...
    \begin{equation}
        ...
        \leq  (k + 2p) \left( 3 \lVert \mtx{A} \rVert _2^2 + \frac{1}{k} \lVert \mtx{A} \rVert _F^2 \right).
    \end{equation}
    %\begin{equation}
    %    \mathbb{E}^{p}\left[ \lVert \mtx{\Sigma} \mtx{\Omega}_2\rVert _2 \right]
    %    &\leq \sqrt{\frac{k + p}{2}} \cdot \left( 2 + \frac{1}{\sqrt{k}} \right) \cdot \lVert \mtx{\Sigma} \rVert _2 + \sqrt{\frac{k + p}{2k}} \cdot \lVert \mtx{\Sigma} \rVert _F. \notag \\
...
\begin{remark}
    \todo{Add remark (need to maybe convert back appendix lemma to $p/2$)}
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{A} \mtx{\Omega} \rVert _2 \right] = \sqrt{\mathbb{E}^{\sfrac{p}{2}}\left[ \lVert \mtx{A} \mtx{\Omega} \rVert _2^2 \right]} \leq \sqrt{k + p} \cdot \left(3 \lVert \mtx{A} \rVert _2 + \frac{1}{\sqrt{k}}\lVert \mtx{A} \rVert _F \right)
    \end{equation}
\end{remark}
...
\begin{proof}
    ...
    \begin{equation}
        \mathbb{E}^p\left[ \lVert \mtx{A} \mtx{\Omega} \rVert _2^2 \right]
        \leq \lVert \mtx{A} \rVert _2^2 \mathbb{E}^p\left[ \lVert \vct{\omega}_1 \rVert _2^2 \right] + \mathbb{E}^p\left[ \lVert \mtx{A} \vct{\omega}_2 \rVert _2^2 \right]
    \end{equation}
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _2 \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _F \right] && \text{($\lVert \mtx{A} \mtx{B} \rVert _F \leq \lVert \mtx{A} \rVert _F \lVert \mtx{B} \rVert _2$)} \notag \\
    ...
    \end{align}
    We can apply \reflem{lem:spectral-norm-moment} to $\mtx{\Lambda}_2^{\sfrac{1}{2}}$ to bound
    \begin{equation}
    ...
    \end{align}
    The last step follows by expanding exponential function $e^n = 1 + n + \dots + n^n/n! + \dots$ for $n \in \mathbb{N}$ to see $e^n \geq 1 + n$ and $e^n \geq n^n / n!$ for all $n \in \mathbb{N}$, from which $(1 + n)^{\sfrac{1}{n}} \leq e$ and $\left( 1/n! \right)^{\sfrac{1}{n}} \leq e/n$ follow respectively. Hence, plugging \refequ{equ:spectral-norm-bound-applied} and  \refequ{equ:pinv-spectral-norm-bound} into \refequ{equ:nystrom-proof-processed-tail} gives us
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _{(4)}^2 \right]
...
This theorem shows that for $\sigma \ll 1$, if we choose $m = \mathcal{O}(\sigma^{-1}(\log(\varepsilon^{-1}) + \log(\sigma^{-1})))$, the error we induce with the Chebyshev expansion is at most $\mathcal{O}(\varepsilon)$.
...
\todo{Summarize analysis (on one hand, half the degree $m$, but easier to interpolate because $\sigma \to \sqrt{2} \sigma$)}
...
\begin{theorem}{Chebyshev-Nyström++ method}{chebyshev-nystrom}
    ...
    \begin{equation}
        ...
        %\label{equ:2-chebyshev-interpolation-error}
    \end{equation}
    for all $0 < \varepsilon \leq 1$ and $\delta \in (0, e^{-\sfrac{1}{2}}/2)$.
\end{theorem}
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq (1 + \varepsilon) 2 \underline{E}_{\sigma, m} + \varepsilon. && \text{(\reflem{lem:non-negative-chebyshev-error} and normalization)} \notag \\
    \end{align}
    From \reflem{lem:non-negative-chebyshev-error} we see that choosing $m = \mathcal{O}(\sigma^{-1}(\log(\varepsilon^{-1}) + \log(\sigma^{-1})))$ ensures $\underline{E}_{\sigma, m} \leq \varepsilon$, and thus, since $\mathcal{\varepsilon} \leq 1$,
    \begin{equation}
        \int_{-1}^{1} \left| \phi_{\sigma}(t) - \widetilde{\phi}_{\sigma}^{(m)}(t) \right|~\mathrm{d}t \leq (1+\varepsilon) 2 \varepsilon + \varepsilon \leq 5 \varepsilon.
    \end{equation}
\end{proof}
...
\begin{lemma}{Moment bound of $\chi^2$-random variable}{gamma}
    Let $X$ follow a $\chi^2$ distribution with $k$ degrees of freedom. Then for all $p \in \mathbb{N}$ it holds
    \begin{equation}
        \mathbb{E}^{p}[X] \leq k + 2p
...
\begin{proof}
    ...
    \begin{equation}
        \mathbb{E}^{p}[X] = 2 \left( \frac{\Gamma(\frac{k}{2} + p)}{\Gamma(\frac{k}{2})} \right)^{\sfrac{1}{p}}.
    \end{equation}
    We can use a telescoping product and the property $\Gamma(z+1)/\Gamma(z) = z$ to bound
    \begin{align}
        \frac{\Gamma(\frac{k}{2} + p)}{\Gamma(\frac{k}{2})} 
        &= \frac{\Gamma(\frac{k}{2} + 1)}{\Gamma(\frac{k}{2})} \frac{\Gamma(\frac{k}{2} + 2)}{\Gamma(\frac{k}{2} + 1)} \cdots \frac{\Gamma(\frac{k}{2} + p)}{\Gamma(\frac{k}{2} + p - 1)} \notag \\
        &= \left(\frac{k}{2}\right) \left(\frac{k}{2} + 1\right) \cdots \left( \frac{k}{2} + p - 1 \right) \notag \\
        &\leq \left(\frac{k}{2} + p - 1\right)^{p} 
    \end{align}
    Hence,
    \begin{equation}
        \mathbb{E}^{p}[X] \leq 2 \left( \frac{k}{2} + p - 1 \right) \leq k + 2p.
    \end{equation}
\end{proof}
