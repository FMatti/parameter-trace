...
Comparing \refthm{thm:nystrom-pp} with its equivalent for constant matrices \cite[theorem 3.4]{persson-2022-improved-variants}, we notice that our choice of $\delta$ is more restricted and the dependence of $n_{\mtx{\Psi}}$ and $n_{\mtx{\Omega}}$ on $\delta$ is worse.
...
\end{equation}
The task in \refequ{equ:smooth-spectral-density} is to approximate the trace of the parameter-dependent matrix $g_{\sigma}(t\mtx{I}_n - \mtx{A})$, which allows us to apply the results derived in \refsec{sec:analysis}.

\todo{
Measuring error between spectral density $\phi$ and approximation $\widetilde{\phi}$
\begin{equation}
    \sup_{f \in \mathcal{S}} \int_{-1}^{1} f(t) (\phi(t) - \widetilde{\phi}(t)) \mathrm{d}t
    \label{equ:error-metric}
\end{equation}

In \cite{lin-2016-approximating-spectral} propose $\mathcal{S} = \{ f: f(t) \equiv g_{\sigma}(t - \lambda), \lambda \in [\lambda_{\text{min}}, \lambda_{\text{max}}]\}$

In \cite{chen-2021-analysis-stochastic} use $\mathcal{S} = \{f : f(t) = \Theta(s - t), s \in [-1, 1] \}$ where $\Theta$ the unit step which is equivalent to Kolmogorov-Smirnov.

In \cite{braverman-2022-sublinear-time} use $\mathcal{S} = \text{Lip}_1 = \{f : |f(x) - f(y)| \leq |x - y| \}$ which is equivalent to the Wasserstein-1 norm.

For small enough $\sigma$ and well separated eigenvalues, the  error \refequ{equ:error-metric} when testing against Lipschitz-1 functions we make by smoothing is already $\mathcal{\sigma}$.

Having noted this, we will from now on measure errors in $L^1$ norm from smooth spectral density.
}
...
\begin{lemma}{Non-negative Chebyshev expansion error}{non-negative-chebyshev-error}
    ...
    \begin{equation}
        \sup_{t \in [-1, 1]} \left| g_{\sigma}(t - s) - \underline{g}_{\sigma}^{(m)}(t - s) \right| \leq 2\sqrt{2} \left(1 + \sigma \sqrt{\pi} \cdot E_{\sqrt{2}\sigma, \sfrac{m}{2}}\right) \frac{E_{\sqrt{2}\sigma, \sfrac{m}{2}}}{n} \equiv \frac{\underline{E}_{\sigma, m}}{n}
        \label{equ:2-chebyshev-interpolation-sup-error-kernel}
        %\lVert  \phi_{\sigma} - \phi_{\sigma}^{(m)} \rVert _1 &\leq \frac{2\sqrt{2}}{\sigma^2} (1 + \sigma)^{-m}.
        %\label{equ:2-chebyshev-interpolation-error}
    \end{equation}
...
\begin{proof}
    ...
    \begin{align}
        ...
        &\leq \left( 2 \frac{1}{\sqrt{n \sigma \sqrt{2\pi}}} + \sqrt{2 n \sigma \sqrt{2 \pi}} \cdot \frac{E_{\sqrt{2}\sigma, \sfrac{m}{2}}}{n}\right)\sqrt{2 n \sigma \sqrt{2 \pi}} \cdot \frac{E_{\sqrt{2}\sigma, \sfrac{m}{2}}}{n},
    \end{align}
