...
\begin{lemma}{Moments of tail bound}{ose}
    Let $\mtx{\Sigma} = \operatorname{diag}(\sigma_1, \dots, \sigma_m) \in \mathbb{R}^{m \times m}$ such that $|\sigma_1| \geq \dots \geq |\sigma_m|$ and independent standard Gaussian random matrices $\mtx{\Omega} \in \mathbb{R}^{m \times k}$ satisfy for all \todo{$k \geq 2$}
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Sigma} \mtx{\Omega} \rVert _2 \right]
...
\begin{proof}
    ...
    \begin{equation}
        ...
        = \mathbb{E}\left[  \left( \lVert \mtx{\Sigma} \vct{\omega} \rVert _2^2 \right)^{\sfrac{p}{2}} \right]^{\sfrac{1}{2p}}
    ...
    Inspired by the proof of \cite[theorem 1]{cohen-2016-optimal-approximate}, we split the diagonal entries of $\mtx{\Sigma}$ into $\ell = \lceil m/k \rceil$ groups of size $k \geq 2$ as follows
    \begin{equation}
    ...
    \end{equation}
    In case $m$ is not a multiple of $k$, we set $\sigma_i = 0$ if $i > m$. Now we see that
    \begin{equation}
        \sigma_1^2 \leq \lVert \mtx{\Sigma} \rVert _2^2
        \quad \text{and} \quad
    ...
    \end{equation}
    By the triangle inequality for moments \todo{[cite Hysan]}
    \begin{align}
        \mathbb{E}^{\sfrac{p}{2}}\left[ \sum_{i=1}^{m} \sigma_i^2 \omega_i^2 \right]
        &= \mathbb{E}^{\sfrac{p}{2}}\left[ \sum_{i=1}^{\ell} \sum_{j=1}^{k} \underbrace{\sigma_{j + (i-1)k}^2}_{\leq \sigma_{1 + (i-1)k}^2} \omega_{j + (i-1)k}^2 \right] \notag \\
        &\leq \mathbb{E}^{\sfrac{p}{2}}\left[ \sum_{i=1}^{\ell} \sigma_{1 + (i-1)k}^2 \sum_{j=1}^{k} \omega_{j + (i-1)k}^2 \right] \notag \\
        &\leq \sum_{i=1}^{\ell} \sigma_{1 + (i-1)k}^2 \cdot \mathbb{E}^{\sfrac{p}{2}}\left[ \sum_{j=1}^{k} \omega_{j + (i-1)k}^2 \right] \notag \\
        &\leq \left( \lVert \mtx{\Sigma} \rVert _2^2 + \frac{1}{k} \lVert \mtx{\Sigma} \rVert _F^2 \right) 2 \left( \frac{\Gamma(\frac{k + p}{2})}{\Gamma(\frac{k}{2})} \right)^{\sfrac{2}{p}} \notag \\
        &\leq \frac{k + p}{2} \left( \lVert \mtx{\Sigma} \rVert _2^2 + \frac{1}{k} \lVert \mtx{\Sigma} \rVert _F^2 \right)
    \end{align}
    Thus, bounding the constant up front with $1$, we get
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Sigma} \vct{\omega} \rVert _2 \right]
        \leq \sqrt{\frac{k + p}{2}} \left( \lVert \mtx{\Sigma} \rVert _2 + \frac{1}{k} \lVert \mtx{\Sigma} \rVert _F \right)
    \end{equation}
    From the proof of \cite[lemma B.1]{tropp-2023-randomized-algorithms} we know that 
    \begin{equation}
        \mathbb{E}^p\left[ \lVert \mtx{\Sigma} \mtx{\Omega} \rVert _2 \right]
        \leq \lVert \mtx{\Sigma} \rVert _2 \mathbb{E}^p\left[ \lVert \vct{\omega}_1 \rVert _2 \right] + \mathbb{E}^p\left[ \lVert \mtx{\Sigma} \vct{\omega}_2 \rVert _2 \right]
    \end{equation}
    for independent standard Gaussian random vectors $\vct{\omega}_1 \in \mathbb{R}^{k}$ and $\vct{\omega}_2 \in \mathbb{R}^{m}$. The second moment is bound by \todo{equation} while for the first moment we apply the above $\mtx{\Sigma} = \mtx{I}_k$ to get
    \begin{equation}
        \mathbb{E}^p\left[ \lVert \mtx{\Sigma} \mtx{\Omega} \rVert _2 \right]
        \leq \sqrt{\frac{k + p}{2}} \cdot \left( 2 + \frac{1}{\sqrt{k}} \right) \cdot \lVert \mtx{\Sigma} \rVert _2 + \sqrt{\frac{k + p}{2k}} \cdot \lVert \mtx{\Sigma} \rVert _F.
    \end{equation}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \leq \sqrt{k} \cdot \mathbb{E}^{\sfrac{k}{2}}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _2^2 \right]
        = \sqrt{k} \cdot \mathbb{E}^{k}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _2 \right]^2
    ...
    \begin{equation}
        \mathbb{E}^{k}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _2 \right]
        \leq \mathbb{E}^{k}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \rVert _2 \lVert \mtx{\Omega}_1^{\dagger} \rVert _2 \right]
        \leq \mathbb{E}^{k}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \rVert _2 \right] \mathbb{E}^{k}\left[  \lVert \mtx{\Omega}_1^{\dagger} \rVert _2 \right]
        \label{equ:OSE-moment-bound-initial-new}
    \end{equation}
    The first term can be bound with \reflem{lem:ose} with $\mtx{\Sigma} = \mtx{\Lambda}_2^{\sfrac{1}{2}}$:
    \begin{equation}
        \mathbb{E}^{p}\left[ \lVert \mtx{\Sigma} \mtx{\Omega} \rVert _2 \right]
        \leq \sqrt{\frac{3}{2}} \cdot \left( 2 \sqrt{k} + \frac{1}{\sqrt{2}} \right) \cdot \lVert \mtx{\Sigma} \rVert _2 + \sqrt{\frac{3}{2}} \cdot \lVert \mtx{\Sigma} \rVert _F.
    \end{equation}
    The moments of $\lVert \mtx{\Omega}_1^{\dagger} \rVert _2$ can be bound with help of the proof of \cite[lemma B.3]{tropp-2023-randomized-algorithms}
    \begin{equation}
    ...
    Just as in the proof of \cite[lemma 3.1]{meyer-2021-hutch-optimal} we can bound
    \begin{align}
