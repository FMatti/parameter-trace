...
\end{equation}
In this case, in the proof of \reflem{lem:sub-exponential-moments} we would not have to pay the factor of $2$ when bounding $\mathbb{P}(|X| > t)$ instead of $\mathbb{P}(X > t)$ with \cite[Proposition 2.9]{wainwright-2019-highdimensional-statistics}. As a consequence, \refequ{equ:sub-exponential-moment-bound} reads
\begin{equation}
    \mathbb{E}[X^{2q}] \leq q! (2 \nu^2)^q + (2q)! (2 \alpha)^{2q}.
\end{equation}
For example, if we choose $\beta = 2$ in \reflem{lem:hutchinson-sub-exponential}, we know that $\Trace(\mtx{A}) - \vct{\psi}^{\top} \mtx{A} \vct{\psi}$ is $(4 \lVert \mtx{A} \rVert _F^2, 4 \lVert \mtx{A} \rVert _2)$-sub-exponential. Thus,
\begin{equation}
