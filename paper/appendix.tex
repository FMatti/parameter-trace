...
In this section we first recall that the (centered) single-query Girard-Hutchinson estimator~\refequ{equ:hutchinson-trace-estimator} is sub-exponential. Combined with a bound on moments of sub-exponential random variables, this will be 
used in the proof of \refthm{thm:hutchinson}.
...
\end{lemma}
\begin{proof}
The statement follows from a straightforward extension of the proof of Theorem 2.3 in~\cite{boucheron-2013-concentration-inequalities} from  even integers $p$ to general $p$. We include the detailed argument for completeness. Using integration by parts, the reparametrization $x = \sqrt{2\nu t} + ct$, and~\eqref{eq:subgammatail} one gets
\begin{align*}
...
\end{align*}
Using $(a+b)^{1/p} \le a^{1/p} + b^{1/b}$, we obtain
\[
