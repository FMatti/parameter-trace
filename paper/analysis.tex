...
The combination suggested in~\cite{lin-2017-randomized-estimation} applies Girard-Hutchinson to correct the error of the Nyström approximation~\eqref{equ:nystrom-approximation}:
\begin{equation}
...
\begin{lemma}{Nyström approximation for parameter-dependent matrices}{nystrom}
    ...
    \end{equation}
        ...
        $c = 108$. In particular, given $\varepsilon > 0$ and $\delta \in (0, 1)$, the bound $\int_{a}^{b} \lVert \mtx{B}(t) - \Nystr{\mtx{\Omega}}{\mtx{B}}(t) \rVert _F~\mathrm{d}t \leq \varepsilon \int_{a}^{b} \Trace(\mtx{B}(t))~\mathrm{d}t$ holds with probability at least $1-\delta$ if $n_{\mtx{\Omega}} = \mathcal{O}(\varepsilon^{-2} + \log(\delta^{-1}))$.
\end{lemma}
...
To treat the second term in~\eqref{equ:nystrom-proof-persson-bonud}, we bound 
its $q$th moment using the independence of $\mtx{\Omega}_1$ and $\mtx{\Omega}_2$:
    \begin{equation}
        ...
        \leq \mathbb{E}^{q}\left[ \sqrt{k} \big\| \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \big\| _2^2 \right]%  && \text{($\lVert \mtx{A} \rVert _{(4)}^2 \leq \sqrt{k} \lVert \mtx{A} \rVert _2^2$)} \notag \\
        ...
        \leq \sqrt{k} \mathbb{E}^{q}\left[ \big\| \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2  \big\|_2^2\right] \mathbb{E}^{q}\left[ \big\| \mtx{\Omega}_1^{\dagger} \big\|_2^2  \right].% && \text{(independence)}
    ...
    \begin{equation} \label{equ:pinv-spectral-norm-bound}
      \mathbb{E}^{\sfrac{n_{\mtx{\Omega}}}{4}}\left[ \big\| \mtx{\Omega}_1^{\dagger} \big\|_2^2 \right]
        ...
        \le \frac{9}{8}  
        \frac{e^2 n_{\mtx{\Omega}}}{(n_{\mtx{\Omega}} / 2+ 1)^2} \le 
        \frac{9}{2}  \frac{e^2}{n_{\mtx{\Omega}}},
    \end{equation}
    where we used $(m!)^{-\sfrac{1}{m}} \leq e/m$. Note that, in contrast 
...
%
    The second factor in~\eqref{equ:nystrom-proof-processed-tail} is bounded using \reflem{lem:spectral-norm-moment} with $\mtx{A} = \mtx{\Lambda}_2^{\sfrac{1}{2}}$: % and $p = n_{\mtx{\Omega}}/2$:
    \[
        \mathbb{E}^{\sfrac{n_{\mtx{\Omega}}}{4}}\left[ \big\| \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \big\|_2^2 \right]
        \leq \textcolor{red}{\frac{3}{2}} n_{\mtx{\Omega}} \Big( \big\| \mtx{\Lambda}_2^{\sfrac{1}{2}} \big\|_2^2 + \frac{1}{n_{\mtx{\Omega}}} \big\| \mtx{\Lambda}_2^{\sfrac{1}{2}} \big\|_F^2 \Big).
    ...
    \begin{equation}
        ...
        \leq \frac{27 e^2}{4\sqrt{2}}  \sqrt{n_{\mtx{\Omega}}} \Big( \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _2^2 + \frac{1}{n_{\mtx{\Omega}}} \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \rVert _F^2 \Big).
    ...
    \begin{equation}
        \mathbb{E}^{\sfrac{n_{\mtx{\Omega}}}{4}}\left[ \lVert \mtx{\Lambda}_2^{\sfrac{1}{2}} \mtx{\Omega}_2 \mtx{\Omega}_1^{\dagger} \rVert _{(4)}^2 \right]
        \leq \frac{27 e^2}{4 \sqrt{2}}  \sqrt{n_{\mtx{\Omega}}} \Big( \frac{2}{n_{\mtx{\Omega}}} \Trace(\mtx{B}) + \frac{1}{n_{\mtx{\Omega}}} \Trace(\mtx{B}) \Big)
        \leq  \frac{106}{\sqrt{n_{\mtx{\Omega}}}} \Trace(\mtx{B}).
    ...
    As in the proof of~\refthm{thm:hutchinson}, the claimed bound~\eqref{qu:nystrompp-theorem-bound}
    follows from~\eqref{eq:blubber} using Minkowski's integral inequality and Markov's inequality.
%     
%     
%         
%     and by the Minkowski's integral inequality \cite[Theorem 202]{hardy-1952-inequalities}, we get for $n_{\mtx{\Omega}} \geq 4$
%     \begin{align}
