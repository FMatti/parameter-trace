...
\begin{remark}
    ...
    \end{equation}
    if $n_{\mtx{\Psi}} \geq 2 c^2 e^2 \log(\delta^{-1}) \varepsilon^{-2} (\int_{a}^{b} \mu(t)^{\sfrac{1}{2}}~\mathrm{d}t)^2$ where $\mu(t) = \lVert \mtx{A}(t) \rVert _2 / \Trace(\mtx{A}(t))$. Up to constant factors, this matches the equivalent result for constant matrices from \cite[Remark 2]{cortinovis-2022-randomized-trace}.
\end{remark}
