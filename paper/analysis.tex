...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{pmatrix}.
    \end{equation}
    Note that $\Trace(\mtx{B}(t)) - \Hutch{\mtx{\Psi}}(\mtx{B}(t)) =  \Trace(\widetilde{\mtx{B}}(t)) - \widetilde{\vct{\psi}}^{\top} \widetilde{\mtx{B}}(t) \widetilde{\vct{\psi}}$, $\lVert \widetilde{\mtx{B}}(t) \rVert _2 = \lVert \mtx{B}(t) \rVert _2 / n_{\mtx{\Psi}}$ and $\lVert \widetilde{\mtx{B}}(t) \rVert _F = \lVert \mtx{B}(t) \rVert _F / \sqrt{n_{\mtx{\Psi}}}$ hold. Therefore, by \reflem{lem:hutchinson-sub-exponential}, $\Trace(\mtx{B}(t)) - \Hutch{\mtx{\Psi}}(\mtx{B}(t))$ is a centered and $(2 \lVert \mtx{B}(t) \rVert _F^2 (1 - 1/\beta)^{-1} / n_{\mtx{\Psi}}, 2 \lVert \mtx{A} \rVert _2 \beta / n_{\mtx{\Psi}})$-sub-exponential random variable for any $\beta > 1$. Thus, we can apply \reflem{lem:sub-exponential-moments} to bound
    \begin{equation}
        \mathbb{E}^{p}[\Trace(\mtx{B}(t)) - \Hutch{\mtx{\Psi}}(\mtx{B}(t))] \leq 2 e^{\sfrac{1}{e}} \left(\frac{\sqrt{\pi}}{1-1/\beta} \sqrt{\frac{p}{n_{\mtx{\Psi}}}} \lVert \mtx{B}(t) \rVert _F + 4 \beta \frac{p}{n_{\mtx{\Psi}}} \lVert \mtx{B}(t) \rVert _2 \right).
    \end{equation}
    Bounding $\lVert \mtx{B}(t) \rVert _2 \leq \lVert \mtx{B}(t) \rVert _F$ and letting $\beta = 1 + \sqrt{\pi} / 2$ we get
    \begin{equation}
        \mathbb{E}^{p}[\Trace(\mtx{B}(t)) - \Hutch{\mtx{\Psi}}(\mtx{B}(t))] \leq \underbrace{8 e^{\sfrac{1}{e}} (1 + \sqrt{\pi} / 2)}_{=~c} \left( \sqrt{\frac{p}{n_{\mtx{\Psi}}}} + \frac{p}{n_{\mtx{\Psi}}} \right) \lVert \mtx{B}(t) \rVert _F.
