...
\textcolor{red}{Moved from below: The downside of the Nyström estimator is that it will -- in general -- not work well enough on matrices whose singular values do not decay quickly, for example $\mtx{B}(t) \equiv \mtx{I}_n$.}
...
Using that $\mtx{B}(t) - \Nystr{\mtx{\Omega}}{\mtx{B}}(t)$ is positive semi-definite (see, e.g.,~\cite[Lemma 2.1]{frangella-2023-randomized-nystrom}) we obtain
    ...
    \end{align*}
    where $\lVert \cdot \rVert _{\ast}$ denotes the nuclear norm and we used a \textcolor{red}{result from the proof of \cite[Corollary 8.2]{tropp-2023-randomized-algorithms} ??? Is this really the original source. Have a look whether David P mentions this somewhere? Also, this may be contained in the original Gittens Mahoney paper but I am not sure ???} \textcolor{green}{The original is in Gittens' arXiv preprint \cite[Theorem 1]{gittens-2011-spectral-norm} but also in Gittens/Mahoney \cite[Equation 4]{gittens-2013-revisiting-nystrom}} in the last equality. This allows us to apply \textcolor{red}{Theorem 9 ??? be careful that the theorem numbering may be different when the paper is published in NLAA ???} \textcolor{green}{changed it to Theorem 5} from \cite[Theorem 5]{kressner-2024-randomized-lowrank}, which immediately implies the statement claimed by the theorem.
\end{proof}

Compared to the constant case~\cite[Theorem 8.1]{tropp-2023-randomized-algorithms}, the bound of Theorem~\ref{thm:nystrom} features an additional factor $1+r$ that cannot be compensated with oversampling.
...
for Hutch++\cite[Theorem 3.1]{meyer-2021-hutch-optimal} and Nyström++\textcolor{red}{??? Include specific result ???}. Indeed, our proof follows the strategy from~\cite{meyer-2021-hutch-optimal}, by first showing that the parameter-dependent Nyström approximation achieves a relative $\varepsilon$-error when using $\mathcal{O}(\varepsilon^{-2})$ queries.
...
Applying Theorem B.1 from~\cite{persson-2023-randomized-lowrank} for $f(x) = x$ yields the bound
    ...
    \end{equation}
    \textcolor{red}{??? STOPPED HERE: Not clear to me why $n_\Omega/4$ should be the optimal exponent for the tail bound ???}
