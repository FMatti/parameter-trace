...
$T_l(s) = \cos(l \cdot \arccos(s))$ for $l = 0,1,\ldots,m$. We choose the coefficients $\mu_l(t)$ in such a way that $g_{\sigma}^{(m)}(t-s)$ interpolates $g_{\sigma}(t-s)$ in the Chebyshev points $s_i = \cos(\pi i / m),~i=0,1,\dots,m$. In this case, $g_{\sigma}^{(m)}$ is often referred to the Chebyshev interpolant \cite[Chapter 2]{trefethen-2020-approximation-theory}.
...
In general, the truncated Chebyshev approximation $g_{\sigma}^{(m)}$ does not inherit the  non-negativity of the smoothing kernel $g_{\sigma}$. Consequently, the matrix $g_{\sigma}^{(m)}(t \mtx{I}_n - \mtx{A})$ may become 
...
In \cite{lin-2017-randomized-estimation}, a slightly different approach to approximating the smoothing kernel $g_{\sigma}$ is taken: They expand $g_{\sigma}$ in an (infinite) Chebyshev series and truncate it after $m$ terms \cite[Chapter 4]{trefethen-2020-approximation-theory}. Subsequently, they approximate the integral representation of the coefficients of the series with a quadrature rule supported in at least $2m + 1$ nodes, which they can compute with the fast Fourier transform (FFT). Our approach and the one from \cite{lin-2017-randomized-estimation} satisfy almost identical convergence bounds \cite[Theorem 8.2]{trefethen-2020-approximation-theory}. However, the procedure from \cite{lin-2017-randomized-estimation} is often significantly slower due to the larger input to the FFT and the implementational disadvantages of the FFT when compared to the DCT. Furthermore, it introduces an additional error when approximating the integral representation of the coefficients with a quadrature. In \reftab{tab:chebyshev-timing-interpolation}, we compare a benevolent implementation of \cite[Algorithm 1]{lin-2017-randomized-estimation} with our approach, both for the standard and the non-negative Chebyshev approximation.%}

\begin{table}[ht]
    \caption{Runtime in milliseconds for computing the coefficients of a Chebyshev approximation with degree $m$ of the smoothing kernel $g_{\sigma}$ in three different ways: Using \cite[Algorithm 1]{lin-2017-randomized-estimation} (\emph{FFT}), applying the discrete cosine transform direclty to $g_\sigma$ (\emph{DCT}), and applying the discrete cosine transform to $\sqrt{g_\sigma}$ together with \refalg{alg:chebyshev-squaring} (\emph{non-negative DCT}). We use $\sigma=0.005$, $n_t=1000$ parameter values, and various degrees $m$. For stability, we average over 7 runs of the algorithms and repeat this $1000$ times to form the mean and standard deviation reported below.}
...
to the non-negative 
Chebyshev approximation  $\mtx{B}(t) = \underline{g}_{\sigma}^{(m)}(t \mtx{I}_n - \mtx{A})$, which yields the approximation
\begin{equation}
...
One usually aims at evaluating~\refequ{equ:chebyshev-nystrom-formula} for many values of $t$. In the following, we explain how this can be done efficiently by exploiting the 
...
also for $l=m+1, \dots, 2m$, this allows us to evaluate $\mtx{K}_2(t)$ cheaply, in an analogous way to $\mtx{K}_1(t)$. Alternatively, \cite{lin-2017-randomized-estimation} proposes to instead approximate the squared smoothing kernel $g_{\sigma}^2$ in a separate "auxiliary" Chebyshev series truncated after degree $2m$. While this direct approximation of $g_{\sigma}^2$ might be more accurate than our squaring approach, it is \emph{inconsistent} in the sense that numerically computing the truncated Chebyshev series of $g_{\sigma}^2$ gives --- in general --- not the same result as squaring the truncated Chebyshev series of $g_{\sigma}$. We have observed that meticulously ensuring consistency between the approximations of $g_{\sigma}$ and $g_{\sigma}^2$ makes a visible difference in terms of accuracy (cf. \reffig{fig:interpolation-issue}). This consistency will also allow us to derive bounds for the estimator \refequ{equ:chebyshev-nystrom-formula}.

\begin{figure}[ht]
    \centering
    \input{plots/interpolation.pgf}
    \caption{Difference in the accuracy when computing the estimate \refequ{equ:cyclic-property} once where $g_{\sigma}^2$ is approximated with an auxiliary truncated Chebyshev series as proposed in~\cite{lin-2017-randomized-estimation} (\emph{inconsistent}), once by squaring the Chebyshev approximation $g_{\sigma}^{(m)}$ using \refalg{alg:chebyshev-squaring} (\emph{consistent}), and once by using the non-negative Chebyshev approximation $\underline{g}_{\sigma}^{(m)}$ \refequ{equ:non-negative-chebyshev-approximation} which is also squared with \refalg{alg:chebyshev-squaring} (\emph{non-negative consistent}). We use a coarse discretization of the Hamiltonian \refequ{equ:5-experiments-electronic-hamiltonian} in two dimensions, fix $n_{\mtx{\Omega}} = 80$, $\sigma = 0.005$, and compute the $L^1$ error from the approximation of the spectral density at $n_t = 100$ uniformly spaced values of $t$ in the interval $[-1, 1]$.}
...
\begin{algorithmic}[1]
    ...
    \Statex \textbf{Parameters:} Even degree of approximationtion $m \in \mathbb{N}$, number of Girard-Hutchinson queries $n_{\mtx{\Psi}} \in \mathbb{N}_0$, Nyström sketch size $n_{\mtx{\Omega}} \in \mathbb{N}_0$,  smoothing parameter $\sigma > 0$
    \Statex \textbf{Output:} Approximate evaluations of the spectral density $\{\widetilde{\underline{\phi}}_{\sigma}^{(m)}(t_i)\}_{i=1}^{n_t}$
    \For {$i = 1, \dots, n_t$}
        \State Compute the coefficients $\{\xi_l(t_i)\}_{l=0}^{m/2}$ of the Chebyshev approximation of $\sqrt{g_{\sigma}}$ \refequ{equ:square-root-chebyshev-approximation}
        \State Compute the coefficients $\{\underline{\mu}_l(t_i)\}_{l=0}^{m}$ of the non-negative Chebyshev approximation by applying \refalg{alg:chebyshev-squaring} to $\{\xi_l(t_i)\}_{l=0}^{m/2}$
        \State Compute the coefficients $\{\underline{\nu}_l(t_i)\}_{l=0}^{2m}$ of the square of the non-negative Chebyshev approximation by applying \refalg{alg:chebyshev-squaring} to $\{\underline{\mu}_l(t_i)\}_{l=0}^{m}$
    \EndFor
...
\begin{theorem}{Chebyshev-Nyström++ method}{chebyshev-nystrom}
    ...
    \begin{equation}
        ...
        %\label{equ:2-chebyshev-interpolation-error}
    \end{equation}
    for all $\varepsilon \in (0, 3)$ and $\delta \in (0, 1/2)$.
\end{theorem}
...
\begin{proof}
    ...
    \end{align}
    From \reflem{lem:non-negative-chebyshev-error} we see that if $0 < \sigma \ll 1$, choosing $m = \mathcal{O}(\sigma^{-1}(\log(\widetilde{\varepsilon}^{-1}) + \log(\sigma^{-1})))$ ensures $\underline{E}_{\sigma, m} \leq \widetilde{\varepsilon}$, and thus, if $\widetilde{\varepsilon} \leq 1$,
    \begin{equation}
