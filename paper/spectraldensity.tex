...
\color{black}
In practice, the $\varepsilon^{-1}$ complexity of $n_{\mtx{\Omega}}$ and $n_{\mtx{\Psi}}$ in \refthm{thm:chebyshev-nystrom} is often pessimistic. The reason is that \refthm{thm:nystrom-pp} does not take advantage of the low-rank structure of the matrix $\mtx{B}(t) = g_{\sigma}(t\mtx{I}_n - \mtx{A})$: The singular values of the matrix $g_{\sigma}(t\mtx{I}_n - \mtx{A})$ are given by
\begin{equation}
    \sigma_i(t) = g_{\sigma}(t - \lambda_{(i)}) = \frac{1}{\sqrt{2 \pi \sigma^2}} e^{-\sfrac{\left(t - \lambda_{(i)}\right)^2}{2 \sigma^2}},
    \label{equ:gaussian-kernel-eigenvalues}
\end{equation}
where $\lambda_{(1)}, \dots, \lambda_{(n)}$ denote the eigenvalues of $\mtx{A}$ sorted by increasing distance from $t$, such that $\sigma_1(t) \geq \dots \geq \sigma_n(t)$. Clearly, the further away an eigenvalue $\lambda_{(i)}$ is from $t$, the (exponentially) smaller its associated singular value $\sigma_i(t)$ will be. Hence, all singular values $\sigma_i(t)$ corresponding to eigenvalues outside of a certain window $[t - d, t + d]$ around $t$, where $d$ is a number depending on $\sigma$ and $n$, will be negligibly small (see \reffig{fig:numerical-rank}).
\begin{figure}[ht]
    \centering
...
\end{figure}
If we choose $n_{\mtx{\Omega}}$ large enough such that the sum $\sum_{i=r+1}^{n} \sigma_i(t)$ which bounds the approximation error in \refthm{thm:nystrom} is negligibly small for all $t$, we achieve a much better approximation. In particular, if we assume the eigenvalues of $\mtx{A}$ to be uniformly distributed, a small calculation shows that $n_{\mtx{\Omega}} = \mathcal{O}(n \sigma)$ will ensure this.
