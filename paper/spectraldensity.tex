...
\end{equation}
\textcolor{red}{??? MOVE BELOW! This is not at a good place
For later purposes, we recall the well-known
...
\textcolor{red}{??? Lines 1 and 2 are quite confusing. Say more explicitly how $\mu$ is computed. Is Algorithm 3.3 applied twice? ???}
...
\begin{proof}
    ...
    \end{equation}
    Since $\sqrt{g_{\sigma}} = \sqrt{2 \sigma \sqrt{2 \pi}} \cdot g_{\sqrt{2}\sigma}$ and $\sqrt{g_{\sigma}} \leq 1/\sqrt{\sigma \sqrt{2 \pi}}$ \textcolor{red}{we can apply the result from \refthm{thm:chebyshev-error} ??? the reference seems to be incorrect ???} to get
    \begin{align}
