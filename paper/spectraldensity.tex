...
\begin{figure}[ht]
    ...
    \caption{Accuracy when computing the estimate \refequ{equ:cyclic-property} in three different ways. \emph{Inconsistent}: Applying an auxiliary truncated Chebyshev series to $g_{\sigma}^2$, as proposed in~\cite{lin-2017-randomized-estimation}. \emph{Consistent}: Squaring the Chebyshev approximation $g_{\sigma}^{(m)}$ using \refalg{alg:chebyshev-squaring}. \emph{Non-negative consistent}: Squaring the non-negative Chebyshev approximation $\underline{g}_{\sigma}^{(m)}$ with~\refalg{alg:chebyshev-squaring}. We use a \textcolor{red}{??? MORE DETAIL: At least, say what is $N$. Same in Figure 3.2 ???} coarse discretization of the Hamiltonian defined in~\refequ{equ:5-experiments-electronic-hamiltonian} in two dimensions, fix $n_{\mtx{\Omega}} = 80$, $\sigma = 0.005$, and compute the $L^1$ error from the approximation of the spectral density at $n_t = 100$ uniformly spaced values of $t$ in the interval $[-1, 1]$.}
...
\begin{algorithmic}[1]
    ...
    \For {$i = 1, \dots, n_t$}
        ...
        \State Compute coefficients $\{\underline{\nu}_l(t_i)\}_{l=0}^{2m}$ of squared non-negative Chebyshev approxim. by applying \refalg{alg:chebyshev-squaring} to $\{\underline{\mu}_l(t_i)\}_{l=0}^{m}$
    \EndFor
...
\begin{theorem}{Error of Chebyshev-Nyström++ estimate}{chebyshev-nystrom}
    For a smoothing parameter $0 < \sigma \ll 1$ \textcolor{red}{??? Why do we need $\ll$ here? This is quite confusing because this is not a mathematical statement. Can this be simply replaced by $<$ or not? ???}, let $\phi_{\sigma}$ be the smoothed spectral density~\eqref{equ:smooth-spectral-density} of a symmetric matrix $\mtx{A} \in \mathbb{R}^{n \times n}$ with spectrum contained in $[-1, 1]$. Consider $\varepsilon \in (0, 3)$ and $\delta \in (0, 1/2)$.
    ...
    \begin{equation}
        ...
        %\label{equ:2-chebyshev-interpolation-error}
    \end{equation}
    with probability at least $1 - \delta$ for $m = \mathcal{O}(\sigma^{-1}(\log(\varepsilon^{-1}) + \log(\sigma^{-1})))$  and $n_{\mtx{\Omega}} = n_{\mtx{\Psi}} = \mathcal{O}(\varepsilon^{-1}\log(\delta^{-1})^2)$.
\end{theorem}

\begin{proof}
Combining results derived above, we obtain that \color{black}
    \begin{align}
    ...
    \end{align}
    From \reflem{lem:non-negative-chebyshev-error} we see that if \textcolor{red}{$0 < \sigma \ll 1$ ??? Why $\ll$? ???}, choosing $m = \mathcal{O}(\sigma^{-1}(\log(\widetilde{\varepsilon}^{-1}) + \log(\sigma^{-1})))$ ensures $\underline{E}_{\sigma, m} \leq \widetilde{\varepsilon}$, and thus, if $\widetilde{\varepsilon} \leq 1$,
    \begin{equation}
