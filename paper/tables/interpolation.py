...
def time_method(method, parameters, num_times=1000, num_repeats=10, num_burnin=0):
    """
    ...
    num_burnin : int >= 0
        The number of times the method is run during the burn-in period.
    ...
    """
    timeit.repeat(lambda: method(**parameters), repeat=1, number=num_burnin)
...
def chebyshev_coefficients_quadrature(t, m, kernel, n_theta=None):
    ...
    mu = np.real(sp.fft.fft(kernel(t, np.cos(theta)), axis=1)[:, :m+1])
...
for i in range(len(methods)):
    for j in range(len(parameters)):
        ...
        mean, error = time_method(methods[i], parameters[j], num_times=1000, num_repeats=7, num_burnin=1000)
        means[i, j] = 1e3 * mean
