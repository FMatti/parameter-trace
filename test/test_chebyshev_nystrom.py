...
import numpy as np
from algorithms.chebyshev_nystrom import chebyshev_approximation, square_chebyshev_approximation
...
def test_chebyshev_approximation():
    ...
    # Test standard Chebyshev approximation of kernel against theoretical bound
    bound = lambda sigma, m: np.sqrt(2 * np.e / np.pi) / sigma ** 2 * (1 + sigma) ** (-m)

    for m in m_list:
        # Compute function values from Chebyshev approximation
        mu = chebyshev_approximation(kernel, t_list, m)
    ...
    # Test non-negative Chebyshev approximation of kernel against theoretical bound
    nonnegative_bound = lambda sigma, m:  2 * np.sqrt(2) * (1 + sigma * np.sqrt(np.pi) * bound(np.sqrt(2)*sigma, m/2)) * bound(np.sqrt(2)*sigma, m/2)

    for m in m_list:
        # Compute function values from Chebyshev approximation
        mu = chebyshev_approximation(kernel, t_list, m, nonnegative=True)
        for i, t in enumerate(t_list):
            truth = gaussian_kernel(t, s_list, sigma=sigma)
            approx = np.polynomial.chebyshev.Chebyshev(mu[i])(s_list)
...
def test_square_chebyshev_approximation():
    np.random.seed(0)
    ...
    for m in m_list:
        ...
        nu_fast = square_chebyshev_approximation(mu)
